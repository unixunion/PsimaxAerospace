...
namespace Receive
{
    ...
    enum Command
    {
        setSAS, // Command to request led to be set in specific state
		setRCS,
		setGEAR,
		setLIGHT,
    };
    ...
    public class Receive
    {
		...
		private bool sas, rcs, gear, light;
        ...
        {
            // Create command
            var command = new SendCommand((int)Command.setSAS, sas);            

            // Send command
            _cmdMessenger.SendCommand(command);

            Console.Write("SAS ");
            Console.WriteLine(sas?"on":"off");

            // Wait for 1 second and repeat
            Thread.Sleep(1000);
            sas = !sas;   // Toggle led state            
        }
